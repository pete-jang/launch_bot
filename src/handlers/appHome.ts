...
export function registerAppHome(): void {
  ...
  app.event("app_home_opened", async ({ event, client }) => {
    try {
      ...
      if (todayOrders.orders.length > 0) {
        const todayBlocks = createTodayOrderBlocks(
          today,
          todayOrders,
          menuSummary,
        );
  ...
  app.action("home_query_week", async ({ ack, body, client }) => {
    ...
    try {
      ...
      // 이번주 주문 내역
      const periodBlocks = createPeriodOrderBlocks(
        "📅 이번주 주문 내역",
        periodSummary,
        start,
        end,
  ...
  app.action(
    ...
    async ({ ack, body, action, client }) => {
      ...
      try {
        ...
        console.log(
          `Admin ${userId} selected date ${selectedDate} for deletion (${orderCount} orders)`,
        );
      } catch (error) {
  ...
  app.view("admin_confirm_delete", async ({ ack, body, view, client }) => {
    ...
    try {
      ...
      console.log(
        `Admin ${userId} deleted ${deletedCount} orders for ${selectedDate}`,
      );
