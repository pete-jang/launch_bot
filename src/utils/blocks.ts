 ...
 * 오늘 주문 내역 블록 생성
 */
export function createTodayOrderBlocks(
  date: string,
  dayOrders: DayOrders,
  menuSummary: any,
): any[] {
 ...
 * 기간별 주문 내역 블록 생성
 */
export function createPeriodOrderBlocks(
  title: string,
  ...
  // 사용자별 주문 현황
  const users = Object.values(period.userSummary).sort(
    (a, b) => b.count - a.count,
  );
  ...
  if (users.length > 0) {
    const userListText = users
      .map((user, index) => {
        const menuText = [];
        if (user.menuBreakdown.가정식 > 0)
          menuText.push(`🍚 ${user.menuBreakdown.가정식}`);
        if (user.menuBreakdown.프레시밀 > 0)
          menuText.push(`🥗 ${user.menuBreakdown.프레시밀}`);
